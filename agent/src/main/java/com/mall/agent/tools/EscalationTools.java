...
public class EscalationTools {
    ...
    public String escalateToHuman(@P(name = "orderId", value = "订单 ID，没有明确订单时传 0") Long orderId,
        ...
        ToolTrace.record("escalate_to_human", ToolTrace.Status.CALLED);
        try {
            EscalationRecord record = EscalationRecord.of(sessionId, orderId, summary);
            records.add(record);
            sink.accept(record);
            log.info("会话升级人工请求已记录");
            ToolTrace.record("escalate_to_human", ToolTrace.Status.OK);
            return "已记录本次升级请求。请引导用户联系人工客服继续处理。";
        } catch (RuntimeException e) {
            ToolTrace.record("escalate_to_human", ToolTrace.Status.ERROR);
            throw e;
        }
    }
