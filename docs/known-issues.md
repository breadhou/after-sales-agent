...
| | |
|---|---|
| **状态** | **待修**（2026-09-22：Task 7 已实现发布端目录指纹；Agent 侧尚无重新拉取、比对并重建索引的消费者。2026-09-19 用户选定方案一的决定保留） |
| **发现于** | 2026-09-19，Task 2 的代码质量审查 |
| **位置** | 计划 A 的 Task 7（`GET /api/after-sales/policies`）与 spec §4.2 |
| **处理时机** | 发布端随计划 A Task 7；消费者在计划 B/C 接入 Agent 索引时落地 |

**现状（2026-09-22）**：Task 7 的 `GET /api/after-sales/policies` 已提供 `{fingerprint, clauses}`；
`PolicyCatalogVO.of` 先复制不可变条款列表，再从这份快照计算指纹。
但 Agent 侧尚未实现重新拉取、比较指纹及按变化重建索引。改动枚举里的条款并重新部署 supermall 后，
如果 Agent 只在启动时建索引，仍会继续引用旧文本。**发布端提供了检测材料，不等于漂移已经被检测和修复。**
...
|---|---|
| **状态** | **待判断** |
| **发现于** | 2026-09-22，整体设计审查，对照现有 supermall 实现 |
| **位置** | `AfterSalesPolicy.java`（27–50 行）；`RefundExecutionServiceImpl.java`（69–108 行）；计划 A「已知简化」 |
| **严重性** | 高——影响「政策解释与实际行为一致」的核心主张 |

**事实**：`SHIPPED_NOT_RECEIVED` 的条款说「退款在货物退回后完成」，规则只看 `SHIPPED/DELIVERED`，执行服务随后直接写退款及订单为 `REFUNDED`，没有退回事实。另两条文本提到「未使用且不影响二次销售」「凭有效凭证」，相应规则也没有这些输入。

**触发与影响**：用户查询已发货订单并退款时，条款解释要求等待退货，后端却按当前简化立即完成。即使 RAG 逐字引用枚举，也会复述与实际执行不符的条件。把文本放在枚举中解决了来源分散，不能自动证明文本语义与判定条件一致。

**建议方向**：优先让条款准确表达当前演示系统实际支持的条件；若坚持这些条件，则必须明确由哪一层提供并核验事实。此处不要求接入真实支付、退货物流或凭证系统，计划 A 已明确接受即时完成等简化。

**与既有条目的区别**：K-12 处理政策优先级与理由区分，K-17 处理文本数字的机械一致性；本条是现有条款承诺了未实现的业务前提，不推翻此前两条的裁定。
...
| **状态** | **待判断** |
| **发现于** | 2026-09-22，整体设计审查 |
| **位置** | spec §10.1（263–265 行）；计划 C 决策提示词（472–487 行）；`AfterSalesPolicy.resolve`；K-30 |
| **严重性** | 中高——影响评测的正确答案与职责边界 |

**事实**：设计把「签收 12 天，用户要求无理由退款」标为 `DECLINE`、禁止 `REFUND`。当前所有 `RECEIVED` 订单都能被 `QUALITY_ISSUE` 兜住，资格接口返回 `eligible=true`；计划 C 又要求资格、政策以系统结果为准，符合条件即调用申请工具。

**影响**：若没有额外明确理由语义，同一用例会同时得到「系统允许」与「评测禁止」两个答案。依照接口执行可能被评测判错；为了通过评测而仅凭天数拒绝，又可能违反 K-30 已澄清的资格契约。不能单凭运行结果反向决定正确答案。

**建议方向**：在建设评测集前明确接口返回的是最终授权，还是仅代表存在候选政策；说明用户主张的理由由哪一层判断，并据此固定用例预期。当前已接受的政策简化是否保留，由用户裁定，不在本轮擅改。
