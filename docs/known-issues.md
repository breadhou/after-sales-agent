...
| | |
|---|---|
| **状态** | **已处理**（2026-09-24，修订设计基线与 Plan C） |
| **发现于** | 2026-09-22，整体设计审查 |
...
**处理（2026-09-24）**：用户裁定复核上下文固定包含四项：同一用户 JWT 下重新读取的可信订单事实与资格事实、未被模型改写的本轮原始用户输入、候选退款动作（订单 ID 与原因）。`RefundReviewContextFactory` 是唯一组装点，直接调用 `get_order` 与 `get_refund_eligibility`；MCP 返回 `isError=true`、空结果或缺失原始输入均 fail-closed 为驳回。复核 Agent 不接工具，也不接收决策 Agent 的思维链、摘要或会话记忆。

**验证要求**：`RefundRequestToolsTest` 必须断言 reviewer 收到原始输入、订单事实、资格事实和候选动作；提示词测试锁住「可信」「原始用户诉求」及排除「推理过程」的约束。正常申请与候选动作遗漏用户施压风险的用例都应覆盖。

### K-35 复核驳回后没有阻止同一申请再次送审

| | |
|---|---|
| **状态** | **已处理**（2026-09-24，用户裁定并修订设计基线与 Plan C） |
| **发现于** | 2026-09-22，整体设计审查 |
...
**裁定与处理（2026-09-24）**：本阶段把「同一申请」定义为**同一会话的同一订单**。首次复核驳回后，可信编排层记录该订单的终止状态并只升级一次；该会话内后续请求直接返回「已记录人工升级请求，请联系人工客服继续处理」，不得再调用 reviewer、executor 或 escalation。即使用户提供新证据，规则仍然成立；本裁定不定义新会话的后续受理策略。

**验证要求**：替身 reviewer 第一次返回驳回、第二次本会返回通过；对同一会话同一订单连续调用两次，断言 reviewer 与 escalation 各调用一次，executor 调用零次。
...
**验证要求**：测试升级返回文本包含「已记录」和「联系人工客服」，且不包含「联系用户」「尽快」或「稍后」。
