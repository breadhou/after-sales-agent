...
## K-48 Task 6 的工具 trace 不绑定目标订单

| | |
|---|---|
| **状态** | **待判断** |
| **发现于** | 2026-09-25，计划 C Task 6 真实运行后的验证记录审查 |
| **位置** | `agent/.../trace/ToolTrace.java` 与 `docs/plans/2026-09-18-plan-c-agent.md` Task 6 |
| **严重性** | 中——影响评测证据对具体订单的归因，不绕过退款执行或复核结构 |

**现状**：`TASK6_TRACE` 只输出固定工具名与状态，例如
`TASK6_TRACE tool=get_refund_eligibility status=ok`，刻意不带参数、订单 ID 或模型内容。因此它只能证明资格工具成功调用，**不能独立证明**决策模型查询的就是 N/A/B/R/P 中的哪一笔。

**当前证据边界**：Task 6 通过同一 JWT 对五笔订单逐笔预检、复核上下文对 `request_refund` 目标订单重新读取并核验事实，以及五笔订单的数据库终态与不变量，共同展示流程正确性；计划文本已明确 trace 的这个限制。它们不能把一条单独的 trace 行提升为「决策模型查了目标订单」的直接证据。

**倾向与处理时机**：留到**阶段 4 评测设计前**决定是否需要加入不泄露用户请求或凭据的目标绑定证据。届时先比较评测的归因需求与最小可观测字段，再决定是否改 trace；当前不自行扩展日志或宣称此问题已解决。
