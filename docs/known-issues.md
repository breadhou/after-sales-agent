...
| **位置** | 计划 C：复核提示词（503–511 行）、`reviewAgent`（1017–1022 行）、`AgentMain` 装配（1090–1093 行） |
| **严重性** | 高——直接影响「独立复核」的有效性 |

**事实**：提示词要求复核订单事实、售后资格、金额、状态及用户诱导，但装配代码只传入「订单号 + 退款原因」。复核 Agent 没有读工具，原因又来自决策模型的工具参数，无法补足订单事实或还原原始用户诉求。

**触发与影响**：决策模型提出错误申请，或把用户施压改写成普通退款原因时，复核没有独立依据判断；严格遵守「拿不准就驳回」会误拦正常申请，直接相信原因又会放过错误申请。工具隔离能保证「经过复核」，不能据此证明「复核有效」。

**建议方向**：由可信编排代码读取订单与资格结果，连同候选动作、必要的原始用户输入构造明确的复核上下文；继续排除决策模型的推理过程。验证应分别覆盖正常申请和被篡改的候选申请。

**范围**：这是尚未执行的 Plan C 的计划缺陷，本轮没有实现或修改该计划。
...
| **状态** | **待判断** |
| **发现于** | 2026-09-22，整体设计审查 |
| **位置** | spec §7.1（190 行）；计划 C 的 `requestRefund`（741–756 行）与重复请求测试（660–670 行） |
| **严重性** | 中高——复核失败后的终止规则只停留在文字 |

**事实**：设计要求「驳回后不重试、不争论，直接升级人工」，但每次 `requestRefund` 都重新调用 reviewer。驳回分支只升级并返回文本，未记录会话/订单的终止状态；测试也只是让 reviewer 永远返回驳回，再断言两次升级。

**触发与影响**：同一申请第一次被驳回后，模型再调一次工具；若第二次复核改判通过，执行分支仍会退款。退款幂等只能防重复写入，无法防止首次驳回后的首次成功执行。

**建议方向**：先明确「本次申请」的边界；在可信编排层保存驳回/已升级状态，重复请求直接返回该状态。用先驳回、后通过的 reviewer 替身验证不会再次送审或执行。是否允许提交新证据后重新申请，另行明确。
...
| **状态** | **待判断** |
| **发现于** | 2026-09-22，整体设计审查 |
| **位置** | 计划 C：`requestRefund`（750–752 行）、`EscalationTools`（787–807 行）、CLI sink（1086–1088 行） |
| **严重性** | 中——工具返回内容会成为用户收到的行动承诺 |

**事实**：工具返回「已转人工」「稍后会有专员联系」「人工客服会尽快跟进」。实际只写进程内列表和日志，CLI sink 只向 stderr 打印，没有可供人工领取的交接流程。

**建议方向**：本阶段可以保留内存与日志的演示实现，但答复应准确说明只记录了升级请求；如要承诺人工跟进，需有最简实际交接出口。这里不要求建设生产工单系统，问题是当前工具返回值超出了实际能力。
...
| **位置** | 计划 C `ReviewVerdict` 代码块（536、547 行） |
| **严重性** | 中——直接阻塞 Plan C Task 3 的交付 |

**事实**：record 组件 `boolean approved` 需要布尔访问器 `approved()`；代码同时声明无参静态工厂 `ReviewVerdict approved()`，名称与签名冲突。

**验证**：将计划 527–550 行的 Java 源码原样提取到系统临时目录，以本机 JDK 22.0.2 的 `javac` 编译，退出码为 1，诊断为 `invalid accessor method in record ReviewVerdict`，并指出访问器返回类型必须匹配 record 组件。

**建议方向**：给静态工厂换名，并同步计划中的调用示例与测试。当前不是执行 Plan C，本轮未改计划，也未据此声称整个模块已构建或测试。
