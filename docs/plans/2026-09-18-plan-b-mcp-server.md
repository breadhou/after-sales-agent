...
/**
 * 政策条款。Agent 侧用它建立检索索引——<b>这是"判定与解释同源"的那条通路</b>。
 *
 * <p>条款文本与判定逻辑出自 supermall 的同一个枚举，因此代码判拒绝时，
 * 检索到的文本不会说"可以退"。</p>
 *
 * <p><b>响应里还带一个指纹</b>（Plan A 的 K-13 修订后，响应体形状为
 * {@code {"code":0,"data":{"fingerprint":"…","clauses":[…]}}}）。它由服务端从
 * <b>本次返回的那批条款</b>派生，条款一改就变。Plan C 侧重新拉取时比对该指纹，
 * 不一致即说明服务端的条款变了，应当重建索引——这是"同源"在<b>时间维度</b>上的保障，
 * 只靠"启动时拉一次"是不够的。</p>
 *
 * <p>本工具<b>原样透传</b>响应体，不做解析——指纹与条款一起交给模型。</p>
 */
...
3. **无缓存** —— `list_policy_clauses` 每次调用都打 supermall。条款很少变动，Plan C 会在启动时拉一次即可。
   ...
   ⚠️ **「拉一次即可」这句在 2026-09-19 被 K-13 修订为有条件的**：启动时拉一次之后，若 supermall 改了条款并重新部署，agent 手里就是**旧文本**，而它对此一无所知——于是可能引用一条已不存在的条款去解释一个按新规则做出的决定。响应里现在带**指纹**（见本计划 Step 4 的 `PolicyTools`），**Plan C 必须实际使用它**（重新拉取时比对，不一致就重建索引），否则这个机制等于没接上。**「拉一次即可」只有在比对指纹的前提下才成立。**
