...
- K-45：supermall `9d45f49` 将入库 datasource/RabbitMQ 密码改为严格环境变量占位。独立临时目录 Maven 打包成功；显式注入本地凭据后，18081 端口出现 `Started MallApplication` 和 Hikari 数据库连接。健康端点未验证 200。原有 8081 进程在本次收尾时仍运行。
...
- 再次开工前重新确认 supermall、MySQL、Redis、RabbitMQ 状态；当前进程和 WSL 会话不保证跨会话存活。启动按两仓 `AGENTS.md`，凭据只经环境变量。
