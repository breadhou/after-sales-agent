...
**计划 A 正在执行中（8 个任务已完成 5 个）**，分支 `feat/after-sales-capability`（在 supermall 仓库），已推送。
...
| # | 阶段 | 产出 | 计划 | 状态 |
|---|---|---|---|---|
| A | supermall 售后能力 | 政策判定 + 资格查询 + 退款执行 + 幂等 | ✅ 已写 | 🔄 **进行中** — Task 1-5 / 8 |
...
> **当前进度：Task 1-5 已完成，从 Task 6（暴露两个 HTTP 端点）接续。**
>
> Task 5 的完整提交链（在 supermall 仓库，按顺序）：
