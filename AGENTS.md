...
> **三份实现计划的全部代码改动都在 supermall，不在本仓库。** 本仓库目前只有文档。
...
**计划 A：Task 1–6 已完成。Task 7 / Task 8 已被用户暂停**（2026-09-19 决定以后再做）。

> ⏸ **不要自行接着做 Task 7 / Task 8。** 计划里的 Task 7/8 文本是**完整的**（Task 7 含 K-13 的指纹方案，
> Task 8 含并发与非法 reason 两步验证），可直接执行，**不必重新设计**。等用户说了再开工。
